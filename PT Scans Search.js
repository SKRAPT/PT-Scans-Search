function init() {
  $ui.register((ctx) => {
    ...
    async function enrichWithChapters(provider, items) {
      ...
      const ok = detailed.filter((entry) => entry.status === "fulfilled").map((entry) => entry.value);
      ...
      try {
        ...
        status.set(enriched.length ? "Concluido" : "Sem resultados");
      } catch (e) {
