function init() {
  $ui.register((ctx) => {
    ...
    panel.channel.on("removeHistory",  (query) => { removeFromHistory(query); });
...
<body>
  ...
  <script>
    ...
    const state = {
      ...
      sourceFilter: "all", activeTab: "search",
      history: [], favorites: []
    };
    function esc(v) {
    ...
    function renderSkeletons() {
      return '<div class="loading-grid">' +
        Array.from({length:6}).map(()=>'<div class="skeleton"></div>').join("") + '</div>';
    }
    ...
    function renderFilters(items) {
      ...
      if (!wrap) return;
      const show = state.activeTab === "search";
      wrap.style.display = show ? "flex" : "none";
      if (!show) return;
      const counts = buildSourceCounts(items);
      const defs = [
        {key:"all",label:"Todos"},{key:"mangaflix",label:"MangaFlix"},
        {key:"mangalivre",label:"MangaLivre"},{key:"hipercool",label:"HiperCool"},
        {key:"tiamanhwa",label:"TiaManhwa"},{key:"mangafire",label:"MangaFire"}
      ];
      wrap.innerHTML = defs.map(d =>
        '<button class="filter-chip '+(state.sourceFilter===d.key?"active":"")+
        '" data-source="'+esc(d.key)+'" type="button">'+
        esc(d.label)+' ('+(counts[d.key]||0)+')</button>'
      ).join("");
      wrap.querySelectorAll(".filter-chip").forEach(btn => {
        btn.addEventListener("click", () => { state.sourceFilter = btn.dataset.source||"all"; render(); });
      });
    }
    function renderSearch() {
      ...
      if (filtered.length===0) return (
        ...
        '<div style="font-size:13px;line-height:1.6;color:#9db1d3;">'+
          (all.length===0 ? 'Pesquisa um título para começar.' : 'Sem resultados para este filtro.')+
        '</div></div></div>'
      );
      return '<div class="grid">'+filtered.map(item => {
        const favClass = isFav(item.id) ? "fav" : "";
        ...
        return (
          '<div class="card">'+
            '<button class="card-fav-btn '+favClass+'" data-id="'+esc(item.id)+'" data-fav="1" title="Favorito">'+favIcon+'</button>'+
            cover+
            '<div class="info">'+
              '<div>'+
                ...
                '<div class="stats">'+
                  ...
                  '<div class="chip '+(item.hasChapters?"ok":"no")+'">Caps: '+(item.hasChapters?"Sim":"Não")+'</div>'+
      ...
      return (
        '<div class="history-wrap">'+
          '<div class="history-header">'+
            ...
            '<button class="btn-sm" id="clearHistBtn">Limpar tudo</button>'+
          '</div>'+
          '<div class="history-list">'+
            hist.map(q =>
              '<div class="history-item" data-hist="'+esc(q)+'">'+
                '<span class="history-item-text">'+esc(q)+'</span>'+
                '<button class="history-del" data-del-hist="'+esc(q)+'" title="Remover">✕</button>'+
              '</div>'
            ).join("")+
          '</div>'+
        '</div>'
    ...
    function renderFavorites() {
      ...
      if (favs.length===0) return (
        '<div class="fav-empty">Sem favoritos ainda.<br/>'+
        '<span style="font-size:11px;opacity:.6">Clica no ☆ em qualquer resultado para guardar.</span></div>'
      );
      return (
        '<div class="fav-header"><h3>⭐ Favoritos</h3></div>'+
        '<div class="fav-grid">'+
          favs.map(f =>
            '<div class="fav-card" data-fav-search="'+esc(f.title)+'">'+
              (f.image
                ? '<img class="fav-cover" src="'+esc(f.image)+'" alt="'+esc(f.title)+'" />'
                : '<div class="fav-cover-placeholder">Sem capa</div>')+
              '<div class="fav-info">'+
                '<div class="fav-title">'+esc(f.title)+'</div>'+
                '<div class="fav-source">'+esc(f.source||"")+(f.year?" · "+esc(f.year):"")+'</div>'+
              '</div>'+
              '<button class="fav-remove" data-rm-fav="'+esc(f.id)+'" title="Remover">✕</button>'+
            '</div>'
          ).join("")+
        '</div>'
      );
    ...
    function render() {
      ...
      app.innerHTML = html;
      app.querySelectorAll("[data-fav='1']").forEach(btn => {
        btn.addEventListener("click", (e) => {
          e.stopPropagation();
          const item = state.results.find(r => r.id === btn.dataset.id);
          if (item) window.webview.send("toggleFavorite", item);
        });
      });
      app.querySelectorAll(".history-item").forEach(row => {
        row.addEventListener("click", (e) => {
          if (e.target.closest("[data-del-hist]")) return;
          state.activeTab = "search";
          window.webview.send("search", row.dataset.hist);
        });
      });
      app.querySelectorAll("[data-del-hist]").forEach(btn => {
        btn.addEventListener("click", (e) => {
          e.stopPropagation();
          window.webview.send("removeHistory", btn.dataset.delHist);
        });
      });
      const clearHistBtn = document.getElementById("clearHistBtn");
      if (clearHistBtn) clearHistBtn.addEventListener("click", () => window.webview.send("clearHistory"));
      app.querySelectorAll("[data-rm-fav]").forEach(btn => {
        btn.addEventListener("click", (e) => {
          e.stopPropagation();
          window.webview.send("removeFavorite", btn.dataset.rmFav);
        });
      });
      app.querySelectorAll("[data-fav-search]").forEach(card => {
        card.addEventListener("click", (e) => {
          if (e.target.closest("[data-rm-fav]")) return;
          state.activeTab = "search";
          window.webview.send("search", card.dataset.favSearch);
        });
      });
    }
    ...
    document.getElementById("query").addEventListener("keydown", (e) => {
      if (e.key==="Enter") { state.activeTab="search"; window.webview.send("search", document.getElementById("query").value); }
    });
