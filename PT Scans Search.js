...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; }
    ...
    .fallback {
      border: 1px solid rgba(255,255,255,.07);
      background: radial-gradient(circle at 30% 20%, rgba(94,162,255,.20), transparent 28%), linear-gradient(180deg, rgba(20,27,43,.95), rgba(9,12,20,.95));
      display: flex;
      align-items: center;
    ...
    .empty {
      ...
      background: radial-gradient(circle at top, rgba(94,162,255,.08), transparent 36%), rgba(255,255,255,.025);
      color: #9db1d3;
      text-align: center;
      padding: 32px;
...
<body>
  ...
  <script>
    const BRAND_ICON = "https://raw.githubusercontent.com/SKRAPT/PT-Scans/main/upscan.png";
    const state = { results: [], status: "Pronto", loading: false, query: "", sourceFilter: "all" };
    ...
    function renderFilters(items) {
      ...
      if (!wrap) return;
      const cnt = buildSourceCounts(items);
      const defs = [
        { key: "all", label: "Todos" },
      ...
      wrap.innerHTML = defs.map(it => '<button class="filter-chip ' + (state.sourceFilter === it.key ? "active" : "") + '" data-source="' + esc(it.key) + '" type="button">' + esc(it.label) + ' (' + (cnt[it.key] || 0) + ')</button>').join("");
    ...
    function render() {
      ...
      if (filtered.length === 0) {
        app.innerHTML = '<div class="empty"><div><div style="font-size:18px;font-weight:800;color:#f4f8ff;margin-bottom:8px;">PT Scans</div><div style="font-size:13px;line-height:1.6;color:#9db1d3;">' + (all.length === 0 ? 'Pesquisa um título para começar.' : 'Não há resultados para este filtro.') + '</div></div></div>';
        return;
      }
      ...
      app.innerHTML = '<div class="grid">' + filtered.map(item => {
        ...
        return '<div class="card">' + cover + '<div class="info"><div><div class="title">' + esc(item.title) + '</div><div class="stats"><div class="chip source">' + esc(item.source || getItemSource(item)) + '</div><div class="chip ' + (item.hasChapters ? 'ok' : 'no') + '">Capítulos: ' + (item.hasChapters ? 'Sim' : 'Não') + '</div><div class="chip">Total: ' + esc(item.chapterCount) + '</div><div class="chip">Último: ' + esc(item.latestChapter || "-") + '</div>' + (item.year ? '<div class="chip">Ano: ' + esc(item.year) + '</div>' : '') + '</div></div><div class="sub">' + esc(item.id || "") + '</div></div></div>';
      }).join("") + '</div>';
    }
